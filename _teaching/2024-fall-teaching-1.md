---
title: "FA24 Fundamentals of Computing Graduate Super TA"
collection: teaching
type: "Undergraduate course"
permalink: /teaching/2024-fall-teaching-1
venue: "University of Notre Dame, Computer Science and Engineering"
date: 2024-08-23
location: "Notre Dame, IN"
---

Graduate "Super" TA for Fundamentals of Computing CSE20311 Fall 2024. 

Responsibilities
======
- Instruct lab sections: 3 per week, 1 hour each 
- Manage undergraduate TAs: Dictate and communicate responsibilities to undergraduate TAs including: communicating what each lab entails and the undergraduate TA's responsibilities during lab, communicate grading responsibilities and rubrics for homework and exams, coordinate TAs to help with proctoring and grading exams. 
- Hold office hours 

Course Details 
======
Topics: 
- Linux commands 
- Programming basics 
- Control flow: conditions, iteration 
- char; C strings; char and string functions
file I/O
- 2D arrays 
- command line arguments (argc/argv), Makefiles 
- structs, struct arrays, reading .csv files 
- pointers, dereferencing, passing by reference 